...
export class LobbyHeroRosterPanelRenderer {
  ...
  render(layout: UiLayout): void {
    ...
    const panelHeight = Math.max(280 * scale, layout.safeHeight - pagePadding * 2);
    ...
    const panelGroup = this.createUiNode('LobbyHeroRosterPanel');
  ...
  private renderFooter(parent: Node, width: number, height: number, scale: number): void {
    ...
    const reload = this.addFooterButton(parent, 'LobbyHeroRosterReloadButton', '刷新', -70 * scale, -height / 2 + 30 * scale, 112 * scale, 36 * scale, scale);
    reload.on(Button.EventType.CLICK, () => this.host.reloadLobbyHeroRoster(), this);
    const close = this.addFooterButton(parent, 'LobbyHeroRosterCloseButton', '返回大厅', 70 * scale, -height / 2 + 30 * scale, 128 * scale, 36 * scale, scale);
    close.on(Button.EventType.CLICK, () => this.host.closeLobbyHeroRosterPanel(), this);
  }
