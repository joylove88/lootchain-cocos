...
当前阶段覆盖 Cocos 登录、资源加载进度页和大厅背景页。

- Cocos Creator 负责登录页完整 UI、背景、动态特效、按钮、弹框、状态、接口联调，以及登录后的资源加载进度页。
- 不再使用 Vue Web/H5 作为登录页验收入口。
- dev-login 成功后进入资源加载进度页，加载 `assets/resources/lobby` 下的大厅背景资源。
