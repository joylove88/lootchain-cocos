...
当前登录页阶段已扩展为 Cocos 登录、资源加载进度页和大厅背景页。背景、天空、龙、人物披风、脚边烟雾、按钮、弹框、输入框、状态提示和登录后的加载流程统一在 Cocos 内实现；HTML/Vue/CSS/H5 不再承担登录页内容或验收。
...
- 大厅当前只展示背景与资源就绪状态，不新增抽卡、英雄、背包、USDT、资金池、链上领取入口。
