...
第一阶段 Vue Web/H5 客户端只开放登录验收：

- `POST /api/player/auth/dev-login`

本文件后续列出的抽卡、英雄、背包接口只是后端已存在的玩家侧契约，不代表第一阶段 UI 可以开放。登录验收通过后，再按新的阶段上下文逐步制作大厅和功能页面。
...
当前默认只读联调，真实抽卡、英雄培养、背包使用/出售等写入口需要专项测试账号和清理策略后再显式接入。
